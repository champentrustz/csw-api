...
while ($row_time = mysqli_fetch_assoc($result_time)) {
    ...
    $sql_num_late = "SELECT * FROM tb_times WHERE tb_time_stucode = '".$row_time['tb_time_stucode']."' and tb_time_type != 1 and tb_time_type_update = 2  and tb_time_semester = '".$semester."' and tb_time_year = '".$year."'";
    $result_num_late = mysqli_query($conn, $sql_num_late);
    $count_late = mysqli_num_rows($result_num_late);

    $sql_rule = "SELECT * FROM tb_rules WHERE tb_student_code = '".$row_time['tb_time_stucode']."' and tb_rule_date = '".$date_day."' and tb_ruletype_id = 1";
    $result_rule = mysqli_query($conn, $sql_rule);
    $count_rule = mysqli_num_rows($result_rule);
    $row_rule = mysqli_fetch_assoc($result_rule);

    if($count_rule > 0){
        $deductionScore = true;
    }
    else{
        $deductionScore = false;;
    }
    ...
    $dataStudent = array(
        ...
        "note" => $row_time['tb_time_reason'],
        "num_late" => $count_late,
        "deduction_score" => $deductionScore,
        "score" => $row_rule['tb_rule_score'],
        "date" => display_date($date_day,'shortthai'),
        "time" => substr($row_time['tb_time_time'],0,5),
